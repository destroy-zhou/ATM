...
class EarlyStopping:
    def __init__(self, accelerator=None, patience=7, verbose=False, save_optim=False, delta=0, save_mode=True):
    ...
    def __call__(self, model_optim, scheduler, val_loss, test_loss, test_mse_loss, model, path, setting):
        score = -val_loss
        setting = '{:.4f}_{:.4f}-{}'.format(test_mse_loss, test_loss, setting)
        if self.best_score is None:
            self.best_score = score
            self.best_setting = setting
            if self.save_mode:
                self.save_checkpoint(model_optim, scheduler, val_loss, model, path)
            ...
            self.best_score = score
            # 删除非空文件夹
            if self.accelerator.is_local_main_process and os.path.exists(path + self.best_setting):
                shutil.rmtree(path + self.best_setting) 
            self.best_setting = setting
            if self.save_mode:
                self.save_checkpoint(model_optim, scheduler, val_loss, model, path)
            self.counter = 0
        return self.best_setting

    def save_checkpoint(self, model_optim, scheduler, val_loss, model, path):
        if self.verbose:
        ...
        save_path = path + self.best_setting
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        state_dict = model.state_dict()
        # 从 state_dict 中删除不需要的参数
        #if not self.save_llm:
        #    keys_to_remove = [key for key in state_dict.keys() if 'llm_model' in key]
        #    for key in keys_to_remove:
        #        del state_dict[key]

        torch.save(state_dict, save_path + '/' + 'checkpoint.pt')
        if self.save_optim:
            self.accelerator.wait_for_everyone()
            if self.accelerator.is_local_main_process:
                print("Saving optimizer...")
                torch.save(model_optim.state_dict(), save_path + '/' + 'optimizer.pt')
...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mse_metric):
    total_loss = []
    total_mse_loss = []
    model.eval()
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            if args.use_amp:
                ...
                if args.output_attention:
                    ...
                    outputs, aux_loss = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, train=False)
